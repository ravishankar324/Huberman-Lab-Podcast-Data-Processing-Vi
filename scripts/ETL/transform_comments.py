...
parser = argparse.ArgumentParser(description="Set start and end suffix for file filtering")
parser.add_argument("--start_suffix", type=int, required=True, help="Start suffix for filtering files")
parser.add_argument("--end_suffix", type=int, required=True, help="End suffix for filtering files")
args = parser.parse_args()


# Function to filter files by suffix
def filter_by_suffix(key, start, end):
    match = re.search(r'_(\d+)\.parquet$', key)
    if match:
...
file_keys = [content['Key'] for content in response.get('Contents', []) if content['Key'].endswith('.parquet')]

start_suffix = args.start_suffix
end_suffix = args.end_suffix

# Filter files by suffix range  
#############################################################################
#############################################################################
selected_keys = [key for key in file_keys if filter_by_suffix(key,start_suffix, end_suffix)]  #### Set the suffix range for comments
#############################################################################
#############################################################################

# Check if there are any files to process
if not selected_keys:
    raise ValueError(f"No files found with suffix in range {start_suffix} to {end_suffix}.")

# Sort the selected keys by suffix
selected_keys.sort(key=lambda x: int(re.search(r'_(\d+)\.parquet$', x).group(1)))

# Construct S3 paths for the selected files
selected_paths = [f"s3a://{bucket_name}/{key}" for key in selected_keys]

# Load the selected files into a Spark DataFrame
for i in selected_paths:
    print(f"Processing file: {i.split('_')[-1]} ##########################")
    ...
    col("snippet.videoId").alias("videoid"),
    col("snippet.topLevelComment.snippet.publishedAt").substr(1, 10).alias("publishedat"),
    trim(col("snippet.topLevelComment.snippet.textOriginal")).alias("textoriginal"),
    col("snippet.topLevelComment.snippet.authorDisplayName").alias("authordisplayName"),
    col("snippet.topLevelComment.snippet.authorProfileImageUrl").alias("authorprofileimageUrl"),
    col("snippet.topLevelComment.snippet.authorChannelUrl").alias("authorchannelurl"),
    col("snippet.topLevelComment.snippet.likeCount").alias("likeCount"),
    col("snippet.totalReplyCount").alias("totalreplycount")
        )
        
    df = df.filter(col("likecount").isNotNull())
    ...
    if df_not_processed.count() > 0:
        df_not_processed \
            ...
            .option("escape", '"') \
            .option("delimiter", ",") \
            .csv(s3_key_notprocessed, mode="overwrite") 
    ...
    if df_processed.count() > 0:
        df_processed \
            ...
            .option("escape", '"') \
            .option("delimiter", ",") \
            .csv(s3_key_processed, mode="overwrite")
